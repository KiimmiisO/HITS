...
class HITS():
	...
	def __init__(self, link_matrix, users, index_id_map, is_sparse=False):
		...
		self.__size = len(index_id_map)

		# self.__names = []
		# for i in range(0, self.__size):
		# 	print("{} : {} : {}".format(i, index_id_map[i], index_id_map[i] in users))
			# self.__names.append(users[index_id_map[i]]['screen_name'])
...
def main():
	...
	sparse_link_matrix_path = '../data/result/sparse_link_matrix_20190614'
	dense_link_matrix_path = '../data/result/dense_link_matrix_20190614'
	if sparse:
	...
	# 		h.plot_graph(i, h.get_names(),1)
		...
		hub_data = []
		auth_data = []
		f_hub = open("hubResult_20190614.txt", "w+")
		for i, k in enumerate(h.get_hubs()):
			print("{}. {} : {}".format(i, k, h.get_names()[i]))
			f_hub.write("{}|{}\n".format(k, h.get_names()[i]))
			hub_data.append((k, h.get_names()[i]))
		f_hub.close
		# print(hub_data)
		hub_data.sort(key=sortFirst, reverse=True)
		# print(hub_data)
		f_hub_data = open("hubData_20190614.txt", "w+")
		for dat in hub_data:
			f_hub_data.write("{}|{}\n".format(dat[0], dat[1]))
		f_hub_data.close

		f_auth = open("../data/result/authsResult_20190614.txt", "w+")
		f_auth_data = open("authData_20190614.txt", "w+")
		for i, k in enumerate(h.get_auths()):
			print("{}. {} : {}".format(i, k, h.get_names()[i]))
			f_auth.write("{}|{}\n".format(k, h.get_names()[i]))
			auth_data.append((k, h.get_names()[i]))
		f_auth.close
		auth_data.sort(key=sortFirst, reverse=True)
		for dat in auth_data:
			f_auth_data.write("{}|{}\n".format(dat[0], dat[1]))
		f_auth_data.close
		# print(h.get_hubs(), h.get_names())
		# print(h.get_auths(), h.get_names())
