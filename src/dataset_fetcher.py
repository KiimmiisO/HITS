...
class ListToMatrixConverter():
	...
	def convert(self):
		...
		for user_id in self._adj_list:
			# if '1001024832312524800' in self._adj_list:
			# 	print("Hello!!!")
			# if '1001024832312524800' == user_id:
			# 	print('Found!!')
			for outdegree in self._adj_list[user_id]['outdegree']:
				self._link_matrix[id_index_map[user_id], id_index_map[outdegree]] = 1
			for indegree in self._adj_list[user_id]['indegree']:
				self._link_matrix[id_index_map[indegree], id_index_map[user_id]] = 1
...
def main():
	...
	key = 'ofkAfCP0Rsc53lm7wHvUdIO2A'
	secret = 'EQAVtoBN019xWVQAp2kkpVEuWKgNFtSlztAkxAHcvv6oInIGhI'
	seed_user = 'KiimmiisO'

	log_path = 'logs.txt'

	users_path = '../data/result/users.pickle'
